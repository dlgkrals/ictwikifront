        ...
        <div className="home-left">
          <div className="notice-section">
            <div className="notice-header">
              <h2>공지사항</h2>
              <Link to="/notices" className="btn btn-secondary">더보기</Link>
            </div>
          ...
          <div className="inquiry-section">
            <div className="inquiry-header">
              ...
              <div className="inquiry-header-actions">
                ...
                <Link to="/inquiries" className="btn btn-secondary">더보기</Link>
              </div>
            </div>
              ...
              <form className="inquiry-form" onSubmit={handleSubmit}>
                <div className="form-grid">
                  <div className="form-group">
                    <label>작업 이름 *</label>
                    <input type="text" value={formData.title} onChange={(e) => handleChange('title', e.target.value)} className="form-input" placeholder="예: IP 주소 설정 요청" />
                  </div>
                  <div className="form-group">
                    <label>요청자 *</label>
                    <input type="text" value={formData.requester} onChange={(e) => handleChange('requester', e.target.value)} className="form-input" placeholder="예: 간호학과 조교" />
                  </div>
                  <div className="form-group form-group-full">
                      ...
                      <div key={gIdx} className="location-group">
                        <div className="location-group-header">
                          ...
                          {formData.locations.length > 1 && (
                            <button type="button" className="btn btn-sm btn-danger" onClick={() => handleChange('locations', formData.locations.filter((_, i) => i !== gIdx))}>건물 삭제</button>
                          )}
                        </div>
                    ...
                    <button type="button" className="btn btn-sm btn-secondary" onClick={() => handleChange('locations', [...formData.locations, emptyLocationGroup])}>+ 건물 추가</button>
                  </div>
                  <div className="form-group">
                    <label>유형</label>
                    <select value={formData.type} onChange={(e) => handleChange('type', e.target.value)} className="form-select">
                      {INQUIRY_TYPES.map((type) => (<option key={type} value={type}>{type}</option>))}
                    </select>
                  </div>
                  <div className="form-group">
                    <label>상태</label>
                    <select value={formData.status} onChange={(e) => handleChange('status', e.target.value)} className="form-select">
                      {statusOptions.map((s) => (<option key={s.code} value={s.code}>{s.displayName}</option>))}
                    </select>
                  </div>
                  <div className="form-group">
                    <label>작업자</label>
                    <select value={formData.workerId ?? ''} onChange={(e) => handleChange('workerId', e.target.value ? Number(e.target.value) : null)} className="form-select">
                  ...
                  <div className="form-group">
                    <label>보조 작업자</label>
                    <select value={formData.subWorkerId ?? ''} onChange={(e) => handleChange('subWorkerId', e.target.value ? Number(e.target.value) : null)} className="form-select">
                      <option value="">선택</option>
                      {staffUsers.map((staff) => (<option key={staff.id} value={staff.id}>{staff.name}</option>))}
                    </select>
                  </div>
                  <div className="form-group">
                    <label>처리 방식</label>
                    <select value={formData.method} onChange={(e) => handleChange('method', e.target.value)} className="form-select">
                      <option value="">선택</option>
                ...
                <div className="form-group">
                  <label>증상 *</label>
                  <textarea value={formData.description} onChange={(e) => handleChange('description', e.target.value)} className="form-textarea" rows={3} placeholder="문제 상황이나 요청 내용을 설명해주세요" />
                </div>
                <div className="form-group">
                  <label>해결 내용</label>
                  <textarea value={formData.solution} onChange={(e) => handleChange('solution', e.target.value)} className="form-textarea" rows={2} placeholder="해결 방법을 입력하세요" />
                </div>
          ...
          <div className="documents-section">
            ...
            <div className="document-category-filters">
              ...
              {DOCUMENT_CATEGORIES.map((cat) => (
                <button key={cat.code} className={`category-filter-btn ${selectedCategory === cat.code ? 'active' : ''}`} onClick={() => setSelectedCategory(cat.code)}>{cat.label}</button>
              ))}
            </div>
